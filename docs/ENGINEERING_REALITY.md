...
**WebGL / cinematic marketing bar (separate from API pilot gate):** `docs/plans/2026-04-04-vex-ELITE-DIGITAL-PRESENCE-v1.md` (**§0–§30**): §21+ (perf), §26–§27 (revenue / investor), **§28** (Cox matrix + **§28.3** automation backlog), **§29** (local IDE **autonomy**), **§30** (**Resource Arsenal** — **GLB drop recipe** + **`NEXT_PUBLIC_HERO_VEHICLE_GLB`** + **`packages/3d-configurator/assets/{models,hdri}`** + Turbo **warm-up** + **`--dry-run=text`** + **`turbo.json`** + **`TURBO_*`** + WebGL profilers); do not confuse with tenant/RBAC correctness above.
