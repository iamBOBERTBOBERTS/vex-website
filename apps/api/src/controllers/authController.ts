...
export async function refresh(req: Request, res: Response) {
  ...
  const user = await systemPrisma.user.findUnique({ where: { id: payload.userId } });
...
export async function me(req: Request, res: Response) {
  ...
  const user = await systemPrisma.user.findFirst({
    where: { id: req.user.userId, tenantId },
  });
...
export async function completeOnboarding(req: Request, res: Response) {
  ...
  await systemPrisma.tenant.updateMany({
    where: { id: effectiveTenantId },
    data: { onboardedAt: new Date() },
  });

  const meUser = await systemPrisma.user.findFirst({
    where: { id: req.user.userId, tenantId: effectiveTenantId },
